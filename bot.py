...
import sys
import signal
import aiohttp # for webhook avatar and keepalive server
...
# --- keepalive & health check web server section ---
...
async def on_ready():
    ...
    try:
        ...
        print(f"\x1b[1;38;2;88;101;242m[discord - commands]\x1b[0m {len(synced)} commands were syncronized.")
    except Exception as e:
...
class SongQueue(ui.LayoutView):
    def __init__(self, vc: wavelink.Player, user):
        ...
        queue = list(vc.queue) if hasattr(vc, "queue") else []
